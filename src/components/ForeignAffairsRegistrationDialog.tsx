...
interface ForeignAffairsRegistrationDialogProps {
  open: boolean;
  onClose: () => void;
}

const ForeignAffairsRegistrationDialog: FC<ForeignAffairsRegistrationDialogProps> = ({ open, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    signup(data).subscribe({
      next: () => {
        ...
        setFormData({ role_id: '2', fullname: '', email: '', phone: '', address: '', password: '' });
        onClose();
      },
      error: (err: Error) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>Register Foreign Affair project</DialogTitle>
      <DialogContent>
        <Box
          component="form"
...
export default ForeignAffairsRegistrationDialog;