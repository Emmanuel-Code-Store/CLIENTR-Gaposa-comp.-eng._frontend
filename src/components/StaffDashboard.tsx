...
const staffStudentTasks = [
  ...
  },
  {
    ...
    status: "History 202",
    topic: "The Cold War",
    taskName: "Research Paper on Cuban Missile Crisis",
  ...
  },
  {
    ...
    taskName: "Prepare Arguments for Debate",
  ...
  },
  {
    ...
    taskName: "Presentation on Civil Rights",
  ...
  },
  {
    ...
    taskName: "Group Project on Industrialization",
    submissionDate: "May 3, 2024",
    action: "Reviewing",
  },
];

const staffElectionCandidates = [
  {
    id: 1,
    name: "Emily Peterson",
...
const staffMessages = [
  ...
  },
  {
    ...
    avatar: "/placeholder.svg?height=40&width=40",
    time: "Yesterday",
    message: "Please review the new curriculum updates",
  },
];
...
const staffActivities = [
  {
    ...
    icon: "vote",
    message: "New candidate nominated for Student Council",
    time: "1 hour ago",
  },
  {
    id: 2,
    type: "Task Graded",
    icon: "grade",
    message: "Graded essays for History 101",
    time: "3 hours ago",
  },
  {
    id: 3,
...
export function StaffDashboard({ user, roleName }: DashboardProps) {
    ...
    <MainContent>
      ...
      <Box sx={{ p: 3 }}>
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          {[
            { title: 'Assigned Students', icon: Users },
            { title: 'Active Classes', icon: Users2 },
            { title: 'Election Votes', icon: Vote },
            { title: 'Total Tasks', icon: FileText },
          ].map((stat) => (
            <Grid item xs={12} md={3} key={stat.title}>
              <StatCard title={stat.title} count={120} icon={stat.icon} />
            </Grid>
          ))}
        </Grid>
            ...
            <Grid item xs={12} md={8}>
              <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
                <Typography variant="h6" component="h1">
                  Student Tasks & Election Nominations
                </Typography>
                ...
                <Table sx={{ minWidth: 650 }}>
                  <TableHead sx={{ backgroundColor: "#f5f5f5" }}>
                    <TableRow>
                      ...
                      <TableCell>Topic/Position</TableCell>
                      <TableCell>Task/Nomination</TableCell>
                      <TableCell>Date</TableCell>
                      <TableCell>Status</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {staffStudentTasks.map((task) => (
                      <TableRow key={task.id} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                        ...
                        <TableCell>{getStatusChip(task.action)}</TableCell>
                      </TableRow>
                    ))}
                    {staffElectionCandidates.map((candidate) => (
                      <TableRow key={candidate.id} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                        ...
                        <TableCell>Votes: {candidate.votes}</TableCell>
                  ...
                  <List sx={{ p: 0 }}>
                    {staffMessages.map((message, index) => (
                      <Box key={message.id}>
                        ...
                        {index < staffMessages.length - 1 && <Divider variant="inset" component="li" />}
                      </Box>
                    ))}
                  </List>
                </CardContent>
                  ...
                  <List sx={{ p: 0 }}>
                    {staffActivities.map((activity, index) => (
                      <Box key={activity.id}>
                        <ListItem alignItems="flex-start" sx={{ px: 0 }}>
                          <ListItemAvatar>
                            <Avatar
                              sx={{
                                bgcolor:
                                  activity.icon === "vote" ? "#ff9800" : activity.icon === "grade" ? "#4caf50" : "#2196f3",
                              }}
                            >
                              <NotificationIcon />
                            </Avatar>
                        ...
                        {index < staffActivities.length - 1 && <Divider variant="inset" component="li" />}
                      </Box>
                    ))}
                  </List>
                </CardContent>
            ...
            <Grid container spacing={3}>
                ...
                <Card>
                  <Box sx={{ p: 3 }}>
                    <Typography variant="h6" sx={{ mb: 3 }}>
                      Assign Task or Nominate for Election
                    </Typography>
                    <Stack spacing={2}>
                      <TextField fullWidth label="Student Name" variant="outlined" size="small" />
                      <Select fullWidth size="small" displayEmpty value="">
                        <MenuItem disabled value="">
                          Select Type (Task/Election)
                        </MenuItem>
                        <MenuItem value="task">Task</MenuItem>
                        <MenuItem value="election">Election Nomination</MenuItem>
                      </Select>
                      <TextField fullWidth label="Topic/Position" variant="outlined" size="small" />
                      <TextField fullWidth label="Description" variant="outlined" size="small" multiline rows={3} />
              ...
              <Grid item xs={12} md={6}>
                <Card>
                  <Box sx={{ p: 3 }}>
                    <Typography variant="h6" sx={{ mb: 3 }}>
                      Submit Grades or Election Results
                    </Typography>
                    <Stack spacing={2}>
                      <TextField fullWidth label="Student/Candidate Name" variant="outlined" size="small" />
                      <TextField fullWidth label="Grade/Votes" variant="outlined" size="small" />
                      <Select fullWidth size="small" displayEmpty value="">
                        <MenuItem disabled value="">
                          Select Category (Grade/Election)
                        </MenuItem>
