...
export default function ClearedParents() {
  const { fetchParentCleared } = useUser();
  ...
  //     setLoading(true);
  //     try {
  //       const parentList = await fetchParentCleared();
  ...
  //         );
  //       } else {
  //         console.error("Unexpected parent data format", parentList);
  ...
  const filteredParents = parents.filter((p) =>
    [p.parentId, p.fullname, p.email, p.roleName, p.userPositions?.join(", ")]
      .join(" ")
      .toLowerCase()
        ...
        <Table>
          ...
          <TableBody>
            {paginatedParents.map((p) => (
              ...
              >
                <StyledTableCell>{p.parentId || "-"}</StyledTableCell>
